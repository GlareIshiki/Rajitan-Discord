...
class SQLiteClient:
    ...
    async def _create_tables(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Add active_persona_id to characters (idempotent)
            try:
                await db.execute(
                    "ALTER TABLE characters ADD COLUMN active_persona_id TEXT NOT NULL DEFAULT ''"
                )
            except Exception:
    ...
    async def get_user_workflow(self, guild_id: str, user_id: str) -> Optional[str]:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                    'SELECT workflow_yaml FROM user_workflows WHERE guild_id = ? AND user_id = ?',
                    (guild_id, user_id)
                ) as cursor:
                    row = await cursor.fetchone()
                    if row and row[0]:
            ...
            logger.error(f"Failed to get user workflow: {e}")
            return None

    async def save_user_workflow(self, guild_id: str, user_id: str, yaml_text: str) -> bool:
        """Save user workflow overlay YAML text"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    '''INSERT INTO user_workflows (guild_id, user_id, workflow_yaml, updated_at)
                       VALUES (?, ?, ?, ?)
                       ON CONFLICT(guild_id, user_id)
                       DO UPDATE SET workflow_yaml = excluded.workflow_yaml,
                                     updated_at = excluded.updated_at''',
                    (guild_id, user_id, yaml_text, datetime.now().isoformat())
                )
                await db.commit()
                return True
        except Exception as e:
    ...
    async def get_guild_personas(self, guild_id: str) -> List[Persona]:
        """Get all personas available to a guild (presets + guild custom)"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                    '''SELECT id, guild_id, name, display_name, description,
    ...
    async def delete_persona(self, persona_id: str) -> bool:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                # Clear active_persona_id references
                await db.execute(
    ...
    async def count_guild_personas(self, guild_id: str) -> int:
        """Count custom personas for a guild"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
